...
async def start_quizz(update: Update, context: CallbackContext):
    ...
    for card in queue:
        ...
        if card.thai_translation == user_response:
            ...
            await update.message.reply_text("Correct! Let's get the next English phrase.")
            
        else:
            # Move the current question to the end of the queue
            queue.append(queue.pop(0))
            await update.message.reply_text("Wrong answer. Let's get the next English phrase.")
